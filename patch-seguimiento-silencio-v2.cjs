...
function insertBeforeExact(marker, block, label) {
  const idx = s.indexOf(marker);
  if (idx < 0) fail(label);
  s = s.slice(0, idx) + block + s.slice(idx);
}

function insertAfterAt(startIdx, marker, block, label) {
  const idx = s.indexOf(marker, startIdx);
  if (idx < 0) fail(label);
...
`;
  ...
  insertBeforeExact(
    marker,
    block,
    "SERVER: no encontre procesarLoteBot"
  );
  console.log("SERVER: motor de seguimiento por silencio preparado");
}

/* 2) Cancelar al entrar mensaje del cliente */
if (!s.includes("SEGUIMIENTO SILENCIO CANCELADO POR RESPUESTA CLIENTE")) {
  const upsertIdx = s.indexOf('sock.ev.on("messages.upsert"');
  if (upsertIdx < 0) fail("SERVER: no encontre messages.upsert");
  ...
  if (!esMio) {
    ...
    "SERVER: no encontre clienteId dentro de messages.upsert"
  );
  console.log("SERVER: cancelacion al responder preparada");
}

/* 3) Programar después de que el bot envía y guarda */
if (!s.includes("await programarSeguimientoSilencio({")) {
  const botFnIdx = s.indexOf("async function procesarLoteBot(lote)");
  if (botFnIdx < 0) fail("SERVER: no encontre procesarLoteBot");

  const marker = `  console.log("BOT RESPONDIO:", {`;
  const idx = s.indexOf(marker, botFnIdx);
  if (idx < 0) fail("SERVER: no encontre BOT RESPONDIO");

  const block = `  await programarSeguimientoSilencio({
    clienteId,
    analisisCRM,
    requiereCloserIA,
  });
...
`;
  ...
  s = s.slice(0, idx) + block + s.slice(idx);
  console.log("SERVER: programacion despues de BOT preparada");
}

/* 4) Mensaje manual desde CRM cancela la secuencia */
if (!s.includes("SEGUIMIENTO SILENCIO CANCELADO POR MENSAJE MANUAL")) {
  const sendIdx = s.indexOf('app.post("/send"');
  if (sendIdx < 0) fail("SERVER: no encontre /send");
...
`,
    "SERVER: no encontre bloque cliente en /send"
  );
  console.log("SERVER: cancelacion por mensaje manual preparada");
}

/* 5) Reloj */
if (!s.includes("MOTOR SEGUIMIENTO SILENCIO ACTIVO")) {
  const listenIdx = s.indexOf("app.listen(PORT");
  if (listenIdx < 0) fail("SERVER: no encontre app.listen");

  const iniciarMarker = "  await iniciarWhatsApp();";
  insertAfterAt(
    listenIdx,
    iniciarMarker,
    `
  ...
  console.log("MOTOR SEGUIMIENTO SILENCIO ACTIVO");`,
    "SERVER: no encontre iniciarWhatsApp dentro de app.listen"
  );
  console.log("SERVER: reloj de seguimiento preparado");
}

/* Escritura atómica: solo guardamos si TODOS los pasos anteriores pasaron */
const tempPath = path + ".tmp-seguimiento";
fs.writeFileSync(tempPath, s, "utf8");
fs.renameSync(tempPath, path);

console.log("PATCH SEGUIMIENTO SILENCIO V2 OK");
